...
static struct workqueue_struct *melfas_ts_upgrade_wq;
...
static void melfas_ts_upgrade(struct work_struct *tmp_work)
{

	struct melfas_ts_data *ts = container_of(tmp_work, struct melfas_ts_data, upgrade_work);
	int i = 1, ret;
...
mms136_fw_read_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (upgrade_flag == 0) {
		...
		if (ts -> version != ts->pdata->fw_ver) {
			queue_work(melfas_ts_upgrade_wq, &ts->upgrade_work);
		}
	} else {
		len = snprintf(buf, PAGE_SIZE, "%d\n", ts->version);
	}
...
static int melfas_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (buf[2] == 'B' && lge_get_board_revno() <= HW_REV_D) {
		MELFAS_binary_nLength = MELFAS_binary_nLength_1;
		MELFAS_binary = MELFAS_binary_1;
	} else {
		MELFAS_binary_nLength = MELFAS_binary_nLength_2; /* for 'C' */
		MELFAS_binary = MELFAS_binary_2;
		ts->pdata->fw_ver = 0xa3;
	}
...
#endif
	melfas_ts_upgrade_wq = create_singlethread_workqueue("melfas_ts_upgrade_wq");
	INIT_WORK(&ts->upgrade_work, melfas_ts_upgrade);
